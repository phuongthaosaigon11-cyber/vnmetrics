...
export async function GET() {
  ...
  try {
    ...
    // DANH SÁCH PROXY ĐỂ LUÂN PHIÊN (Chiến thuật: Chết cái này nhảy sang cái khác)
    const proxies = [
        // Cách 1: CodeTabs Proxy (Thường xuyên xuyên qua được tường lửa)
        `https://api.codetabs.com/v1/proxy?quest=${encodeURIComponent(targetUrl)}`,
        // Cách 2: AllOrigins (Dự phòng)
        `https://api.allorigins.win/raw?url=${encodeURIComponent(targetUrl)}`,
        // Cách 3: CorsProxy (Cũ, dùng để vét)
    ...
    if (items.length > 0) {
        ...
        return NextResponse.json([
            { name: 'Lỗi: Tất cả Proxy đều bị chặn', buy: 0, sell: 0, change: 0, type: 'ERROR' }
        ]);
    }
