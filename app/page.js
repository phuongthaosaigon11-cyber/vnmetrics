...
export default function Home() {
  ...
  const fetchAllData = async () => {
    ...
    // 2. ETF (CoinGlass Proxy - Lấy từ file route.js bạn vừa tạo)
    try {
       const etfRes = await fetch('/api/etf-flow'); // Gọi về server của mình
       const etfJson = await etfRes.json();
       
       if (etfJson.data && Array.isArray(etfJson.data)) {
          // Lọc lấy các mã ETF Bitcoin Spot nổi tiếng
       ...
       newErrors.etf = "Không thể lấy dữ liệu CoinGlass lúc này.";
    }
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-[#0f111a] border border-[#2B313F] p-3 rounded-lg shadow-xl text-xs">
          <div className="text-slate-400 mb-2">{payload[0].payload.fullTime}</div>
          <div className="text-white font-bold mb-1">Price: {formatPrice(payload[0].payload.price)}</div>
  ...
  return (
    <div className={`min-h-screen bg-[#F8FAFC] text-slate-900 ${inter.className} pb-10`}>
      {showConsent && (
        <div className="fixed bottom-0 inset-x-0 bg-white border-t border-slate-200 shadow-xl z-[100] p-6 flex justify-between items-center">
      ...
      <div className="bg-slate-900 text-slate-400 text-[11px] py-2 border-b border-slate-800 px-4 flex justify-between">
         ...
         <div className="flex items-center gap-2"><span>Real Data (CoinGlass & Binance)</span><div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></div></div>
      </div>
      ...
      <nav className="bg-white border-b sticky top-0 z-50 shadow-sm px-4 h-16 flex justify-between items-center">
         ...
         <div className="hidden md:flex bg-slate-100 p-1 rounded-lg">
            {['market', 'etf', 'dex'].map(t => (
               <button key={t} onClick={() => setActiveTab(t)} className={`px-4 py-1.5 text-sm font-bold rounded-md capitalize transition ${activeTab===t ? 'bg-white shadow text-blue-700' : 'text-slate-500'}`}>{t === 'etf' ? 'ETF CoinGlass' : t}</button>
            ))}
         </div>
         <div className="flex gap-2">
            <button onClick={() => setCurrency(currency==='USD'?'VND':'USD')} className="px-3 py-1.5 bg-slate-100 rounded text-xs font-bold border w-16">{currency}</button>
            <button className="bg-slate-900 text-white px-4 py-1.5 rounded text-sm font-bold flex items-center gap-2"><Lock size={14}/> Login</button>
         </div>
      </nav>
                ...
                <div className="lg:col-span-2 bg-white p-6 rounded-2xl border shadow-sm h-[500px] flex flex-col">
                   <div className="flex justify-between mb-4">
                      <div className="flex items-center gap-4">
                         <img src={selectedCoin.image} className="w-10 h-10 rounded-full"/>
                   ...
                   <div className="flex-grow w-full">
                      ...
                      ) : <div className="h-full flex items-center justify-center text-slate-400 border border-dashed rounded">Đang tải biểu đồ Binance...</div>}
                   </div>
                </div>
           ...
           <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm min-h-[400px]">
              ...
              {errors.etf ? (
                 <div className="text-center py-10">
                    ...
                    <p className="text-sm text-slate-400 mt-2">Vui lòng kiểm tra file "app/api/etf-flow/route.js" đã được tạo chưa.</p>
                 </div>
              ) : (
      ...
      <footer className="bg-slate-900 text-slate-400 py-12 border-t border-slate-800 mt-16 text-center text-[10px]">
        <p>&copy; 2026 VNMetrics. Data: CoinGlass (ETF), Binance (Chart), DefiLlama (DEX), CoinGecko (Price).</p>
      </footer>
    </div>
  );
}