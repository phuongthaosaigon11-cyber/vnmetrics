      ...
      return (
        <div className="bg-[#0f111a] border border-[#2B313F] p-3 rounded-lg shadow-xl text-xs min-w-[180px] z-50">
          ...
          <div className="flex justify-between items-center mb-1 gap-4">
             ...
             <span className={`text-white font-bold ${jetbrainsMono.className}`}>{formatPrice(d.price)}</span>
          </div>
          <div className="flex justify-between items-center gap-4">
             <span className="text-slate-400">Vol</span>
             <span className={`text-slate-300 font-bold ${jetbrainsMono.className}`}>{formatCompact(d.volume)}</span>
          </div>
        </div>
      );
    }
