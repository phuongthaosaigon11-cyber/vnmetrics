...
import { 
  ComposedChart, Area, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, ReferenceLine, Line
} from 'recharts';
import { 
...
// Import các Component chuyên sâu đã xây dựng trước đó
...
export default function VNMetricsDashboard() {
  ...
  // STATE CHO MARKET TAB (Logic Chart đẹp)
  ...
  return (
    <div className={`min-h-screen bg-[#F8FAFC] text-slate-900 ${inter.className} pb-10`}>
      ...
      <nav className="bg-white border-b sticky top-0 z-50 shadow-sm px-4 h-16 flex justify-between items-center backdrop-blur-md bg-white/95">
         ...
         <div className="hidden md:flex bg-slate-100 p-1 rounded-lg">
            {[{id:'MARKET',l:'Thị trường'},{id:'ETF',l:'Smart Money'}, {id:'METALS', l:'Vàng & Bạc'}].map(t => (
               <button key={t.id} onClick={() => setActiveTab(t.id as any)} className={`px-5 py-1.5 text-sm font-bold rounded-md capitalize transition-all ${activeTab===t.id ? 'bg-white shadow text-blue-600' : 'text-slate-500 hover:text-slate-900'}`}>{t.l}</button>
            ))}
         </div>
      ...
      {/* --- TAB 1: MARKET (CHART ĐẸP CỦA BẠN) --- */}
      {activeTab === 'MARKET' && (
        <div className="animate-in fade-in">
          <div className="bg-white border-b py-8 px-4 bg-gradient-to-b from-white to-slate-50">
             {status.market === 'error' ? 
               <div className="text-center text-red-500 text-sm py-4"><ServerCrash className="mx-auto mb-2"/>Lỗi kết nối CoinGecko.</div> 
             : 
                ...
                cryptos.slice(0, 5).map(c => {
                   ...
                   return (
                     <div key={c.id} onClick={() => handleSelectCoin(c)} 
                        ...
                        <div className={`text-lg font-bold ${jetbrainsMono.className} text-slate-900`}>{formatPrice(c.current_price, currency)}</div>
                     </div>
                   );
                })}
             </div>}
                  ...
                  <div className="flex flex-col md:flex-row justify-between mb-6 gap-4">
                    <div className="flex items-center gap-4">
                       ...
                       <div>
                         ...
                         <div className="flex items-center gap-3 mt-1"><span className={`text-2xl font-bold ${jetbrainsMono.className} text-slate-900`}>{formatPrice(selectedCoin.current_price, currency)}</span></div>
                       </div>
                    </div>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {cryptos.map((coin) => (
                      <tr key={coin.id} onClick={() => handleSelectCoin(coin)} className={`hover:bg-slate-50 cursor-pointer transition ${selectedCoin?.id === coin.id ? 'bg-blue-50/50' : ''}`}>
                        ...
                        <td className={`px-6 py-4 text-right font-bold ${jetbrainsMono.className}`}>{formatPrice(coin.current_price, currency)}</td>
      ...
      {/* --- TAB 2: ETF (SMART MONEY DASHBOARD ĐEN HUYỀN BÍ) --- */}
      {activeTab === 'ETF' && (
        <div className="bg-[#0B0E14] min-h-screen p-4 md:p-6 animate-in fade-in">
